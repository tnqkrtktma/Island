        ...
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            if (newSlot != null)
            {
                ...
                if (Inventory.GetComponent<CraftSystem>() != null)
                {
                    ...
                    if (newSlotChildCount != 0 && isOnSlot)
                    {
                        if(newSlot.parent.tag=="CraftClone")
                        {
                            //check sameitem 
                            if (firstItem.itemID == secondItem.itemID)
                            {
                                //check amount
                                if (firstItem.itemValue == secondItem.itemValue)
                                {
                                    Destroy(secondItemGameObject);
                                    firstItemGameObject.transform.SetParent(secondItemGameObject.transform.parent);
                                    firstItemRectTransform.localPosition = Vector3.zero;
                                    firstItemGameObject.GetComponent<ConsumeItem>().enabled = false;
                                    firstItemGameObject.GetComponent<DragItem>().enabled = false;
                                }
                                else if (firstItem.itemValue > secondItem.itemValue)
                                {
                                    Destroy(secondItemGameObject);

                                    GameObject newgo = (GameObject)Instantiate(Resources.Load("Prefabs/Item") as GameObject);
                                    newgo.GetComponent<ItemOnObject>().item = firstItem.getCopy();
                                    newgo.GetComponent<ItemOnObject>().item.itemValue = firstItem.itemValue - secondItem.itemValue;
                                    newgo.transform.SetParent(oldSlot.transform);
                                    newgo.GetComponent<RectTransform>().localPosition = Vector3.zero;
            ...
            {
                GameObject dropItem = (GameObject)Instantiate(GetComponent<ItemOnObject>().item.itemModel); // name이나 설정된 걸로 바꿔주면 아이템 드랍으로 바뀜
                dropItem.AddComponent<PickUpItem>();
                dropItem.GetComponent<PickUpItem>().item = this.gameObject.GetComponent<ItemOnObject>().item;               
                dropItem.transform.localPosition = GameObject.FindGameObjectWithTag("Player").transform.localPosition + new Vector3(0, 0, 0.5f);
                inventory.OnUpdateItemList();
