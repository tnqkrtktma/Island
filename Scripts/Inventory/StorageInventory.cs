...
public class StorageInventory : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distance <= distanceToOpenStorage && Input.GetKeyDown(inputManagerDatabase.StorageKeyCode))
        {
            ...
            Inventory mainInventory = GameObject.FindGameObjectWithTag("Player").GetComponent<Player>().inventory.GetComponent<Inventory>();
            if (mainInventory != null && !mainInventory.gameObject.activeSelf)
