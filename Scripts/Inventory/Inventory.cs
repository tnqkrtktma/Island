...
public class Inventory : MonoBehaviour
{
    ...
    {
        if (this.tag == "CraftSystem")
            GetComponent<CraftSystem>().backToInventory();
        this.gameObject.SetActive(false);
        checkIfAllInventoryClosed();
    }
    public void craftcloseInventory()
    {
        this.gameObject.SetActive(false);
        checkIfAllInventoryClosed();
    }
    ...
    public void addItemToInventory(int id)
    {
        ...
        if (check)
        {
            GameObject dropItem = (GameObject)Instantiate(itemDatabase.getItemByID(id).itemModel);
